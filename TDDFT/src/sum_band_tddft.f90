...
!
!----------------------------------------------------------------------------
SUBROUTINE sum_band_tddft(evcfile,rho)
  !----------------------------------------------------------------------------
  !
  ...
  USE fixed_occ,            ONLY : one_atom_occupations, tfixed_occ
  ...
  USE wvfct,                ONLY : nbnd, npwx, npw, igk, wg, et, btype, ecutwfc, g2kin
  ...
  type (scf_type) :: rho
! For reading the evc of TDDFT, stored at a different file
  !
  !
  ...
  IF ( gamma_only ) THEN
     !
     CALL sum_band_gamma_tddft(evcfile)
     !
  ELSE
     !
         CALL sum_band_k_tddft(evcfile)
     !
  END IF
  ...
  CONTAINS
     ...
     !
     !-----------------------------------------------------------------------
     SUBROUTINE sum_band_gamma_tddft(evcfile)
       !-----------------------------------------------------------------------
       !
       ! ... gamma version
       !
       ...
       IF( dffts%have_task_groups ) THEN
          ...
          IF( dft_is_meta() .OR. lxdm) &
             CALL errore( ' sum_band_tddft ', ' task groups with meta dft, not yet implemented ', 1 )
          !
       ...
       k_loop: DO ik = 1, nks
          ...
          IF ( nks > 1 ) THEN
             ...
             READ( iunigk ) igk
!          END IF
! AK TESTING CHANGE BACK
             CALL get_buffer ( evc, nwordwfc, evcfile, ik )
             !
          END IF
          !
          IF ( nkb > 0 ) &
       ...
       RETURN
       !
     END SUBROUTINE sum_band_gamma_tddft
     !
     !
     !-----------------------------------------------------------------------
     SUBROUTINE sum_band_k_tddft(evcfile)
       !-----------------------------------------------------------------------
       !
       ! ... k-points version
       !
...
!        ! read wfcs from file and compute becp
           call get_buffer (evc_temp, nwordwfc, evcfile, ik)
!*******************************************************************
          ! ... here we compute the band energy: the sum of the eigenvalues
          !
          DO ibnd = 1, nbnd, incr
                      ...
                      IF( idx + ibnd - 1 <= nbnd ) THEN
                         DO j = 1, npw
                            tg_psi_nc( nls( igk( j ) ) + ioff, 1 ) = &
                                                       evc_temp( j, idx+ibnd-1 )
                            tg_psi_nc( nls( igk( j ) ) + ioff, 2 ) = &
                                                       evc_temp( j+npwx, idx+ibnd-1 )
                         END DO
                      END IF
...
!
                   psic_nc = (0.D0,0.D0)
                   DO ig = 1, npw
                      psic_nc(nls(igk(ig)),1)=evc_temp(ig     ,ibnd)
                      psic_nc(nls(igk(ig)),2)=evc_temp(ig+npwx,ibnd)
                   END DO
...
!$omp do
                         DO j = 1, npw
                            tg_psi( nls( igk( j ) ) + ioff ) = evc_temp( j, idx+ibnd-1 )
                         END DO
!$omp end do
                      END IF
...
!$omp end parallel
                ...
                ELSE
                   ...
                   psic(:) = ( 0.D0, 0.D0 )
                   !
                   psic(nls(igk(1:npw))) = evc_temp(1:npw,ibnd)
                   !
                   CALL invfft ('Wave', psic, dffts)
                   !
                   ! ... increment the charge density ...
...
!$omp end parallel
       ...
       RETURN
       !
     END SUBROUTINE sum_band_k_tddft
     !
     !
