...
!
!----------------------------------------------------------------------------
SUBROUTINE h_psi_deltat( lda, n, m, vrs, v, psi, hpsi)
  !----------------------------------------------------------------------------
  !
  ...
  real(dp), intent(inout) :: vrs(dfftp%nnr, nspin)
  !
  ...
  IF ( lda_plus_u .AND. U_projection.NE."pseudo" ) THEN
     !
     IF (noncolin) THEN
        CALL vhpsi_nc_deltat( lda, m, psi, hpsi, v )
     ELSE
        call vhpsi_deltat( lda, n, m, psi, hpsi, v )
     ENDIF
     !
  ENDIF
  !
  ...
  !
  ! CAUTION, NOT MODIFIED YET FOR DELTAT VERSION
  IF ( lelfield ) THEN
         call errore ('hpsi_deltat', 'lelfield not combinable with predictor-corrector', 1)
     !
